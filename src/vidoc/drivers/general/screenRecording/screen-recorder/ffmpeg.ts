...
export class FFmpegUtil {
  ...
  private static getAll(
    src: any,
    target: any,
    keys: string[] = Object.keys(target),
    override?: (x: string) => string
  ) {
  ...
  static async getWin32Args(opts: RecordingOptions) {
    ...
    if (opts.audio) {
      out.push(...getAll(this.recordingArgs.audio));
    }
  ...
  static async getDarwinArgs(opts: RecordingOptions) {
    const {
      window: { bounds, screens },
    } = opts;

    const getAll = this.getAll.bind(this, opts.ffmpeg!.flags || {});
    const devs = await OSUtil.getMacInputDevices(
      opts.ffmpeg!.binary!,
      opts.window,
      opts.audio
    );
    ...
    const screen = screens.find(
      (
        s // Grab screen which has the top-left corner
      ) =>
        bounds.x >= s.x &&
        bounds.x <= s.x + s.width &&
        bounds.y >= s.y &&
        bounds.y <= s.y + s.height
    )!;
    ...
    out.push(
      ...getAll(this.recordingArgs.video),
      ...getAll({
        vf: `'scale=${screen.width}:${screen.height}:flags=lanczos,crop=${
          bounds.width
        }:${bounds.height}:${Math.abs(screen.x - bounds.x)}:${Math.abs(
          screen.y - bounds.y
        )}'`,
      })
    );
  ...
  static async getX11Args(opts: RecordingOptions) {
    ...
    out.push(...getAll(this.recordingArgs.video));

    return out;
  }

  static async findFFmpegBinIfMissing<
    T extends { ffmpeg?: { binary?: string } }
  >(opts: T): Promise<T & { ffmpeg: { binary: string } }> {
    if (!opts.ffmpeg || !opts.ffmpeg.binary) {
      opts.ffmpeg = opts.ffmpeg || {};
      opts.ffmpeg.binary = await OSUtil.findFileOnPath(
  ...
  static async startRecording(
    ...
    switch (process.platform) {
      case "win32":
        args = await this.getWin32Args(opts);
        break;
      case "darwin":
        args = await this.getDarwinArgs(opts);
        break;
      case "linux":
      case "freebsd":
      case "openbsd":
    ...
    const { finish, kill, proc } = await Util.processToPromise(
      opts.ffmpeg.binary,
      [...args, opts.file.includes(" ") ? `"${opts.file}"` : opts.file]
    );
    return {
  ...
  static async generateGIF(options: GIFOptions): Promise<GIFResult> {
    ...
    let vf = `fps=${opts.fps}`;
    if (opts.scale) {
      vf = `${vf},scale=${Math.trunc(bounds.width * opts.scale)}:${Math.trunc(
    ...
    const paletteFile = path.resolve(
      os.tmpdir(),
      `palette-gen.${Math.random()}.${Date.now()}.png`
    );
    const final =
      opts.output ||
      (/.mp4$/.test(opts.file)
        ? opts.file.replace(".mp4", ".gif")
        : `${opts}.gif`);
    ...
    const { finish, kill } = Util.processToPromise(ffmpeg, [
      ...
      paletteFile,
      "-lavfi",
      `"${vf},paletteuse=dither=bayer:bayer_scale=5:diff_mode=rectangle"`,
      "-y",
      final,
    ]);
