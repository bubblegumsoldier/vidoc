...
export class CLIFileController implements FileController {
    ...
    async writeFileContent(filePath: string, content: string, relative: boolean): Promise<void> {
        const resolvedPath = relative ? path.resolve(process.cwd(), filePath) : filePath;
    ...
    async copyFile(sourceFilePath: string, targetFilePath: string, relative: boolean): Promise<void> {
        const resolvedSource = relative ? path.resolve(process.cwd(), sourceFilePath) : sourceFilePath;
        const resolvedTarget = relative ? path.resolve(process.cwd(), targetFilePath) : targetFilePath;
    ...
    async getAllFilesInFolder(folderPath: string, relative: boolean): Promise<string[]> {
        const resolvedPath = relative ? path.resolve(process.cwd(), folderPath) : folderPath;
